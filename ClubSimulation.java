...
public class ClubSimulation {
   ...
   //MODIFICATION
   static Semaphore entranceSemaphore;
   static Semaphore clubLimitSemaphore;
   ...
   //
	...
	public static void setupGUI(int frameX,int frameY,int [] exits) {
		...
		startB.addActionListener(new ActionListener() {
            ...
            clubView.run(); // Start the animation
            counterDisplay.run(); // Start updating counters	  
		    }
		   });
			...
			pauseB.addActionListener(new ActionListener() {
		      public void actionPerformed(ActionEvent e) {
               ...
               synchronized (pauseLock) {
                ...
                if (!isPaused.get()) {
                    pauseLock.notifyAll(); // Resume threads
                }
            }
		      }
		    });
	...
	public static void main(String[] args) throws InterruptedException {
		...
		//MODIFICATION: ENTRANCE AND CAPACITY
      entranceSemaphore = new Semaphore(1); // Entrance door semaphore
      clubLimitSemaphore = new Semaphore(max); // Club limit semaphore
      // 	
