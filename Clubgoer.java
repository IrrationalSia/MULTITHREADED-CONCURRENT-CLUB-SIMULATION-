...
public class Clubgoer extends Thread {
	...
	Clubgoer( int ID,  PeopleLocation loc,  int speed, Object pauseLock, AtomicBoolean isPaused) {
		this.ID=ID;
    ...
    synchronized (club.barman) {
        while (club.barman.isBusy()) {
            club.barman.wait(); // Wait for the drink to be ready
        }
        club.barman.takeOrder();
        club.barman.notify();
